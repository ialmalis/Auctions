...
class Receive_Proposals extends Behaviour {
    ...
    private String product_name;
    ArrayList<Integer> tempPrices=new ArrayList<Integer>();
    ...
    public void action() {
        ...
        if (testAgent.ProposalsSent) {
            ...
            if (Counter == testAgent.myBidders.length) {
                ...
                if (refusals<Counter) {
                tempPrices.sort(Collections.reverseOrder());
                if (refusals==Counter-1)
                {
                	testAgent.won_price=tempPrices.get(0);
                }
                else 
                {
                	testAgent.won_price=tempPrices.get(1);
                }
