...
public class Bidder extends Agent {
	...
	protected void setup() {
		...
		this.addBehaviour(new receiveProposal(this));

		// Winner
		this.addBehaviour(new Winner(this));

		// Add the behaviour for receiving INFORM
		this.addBehaviour(new Inform(this));
	}
...
class receiveProposal extends CyclicBehaviour {

    private Bidder testAgent;

    public receiveProposal(Bidder agent) {
        super(agent);
        testAgent = agent;
    }
    ...
    public void action() {
        ...
        if (msg != null) {
            ...
            //bid a little bit higher than belief
            testAgent.belief=(int)(testbelief*1.0);
...
class Winner extends CyclicBehaviour {

    private Bidder testAgent;

    public Winner(Bidder agent) {
        super(agent);
        testAgent = agent;
    }
    ...
    public void action() {
        ...
        if (msg != null) {
            ...
            System.out.println("remaining budget of "+testAgent.getAID().getLocalName()+" is: "+testAgent.budget);
        }
        else {
            block();
        }
...
class Inform extends CyclicBehaviour {
	 private Bidder testAgent;

	    public Inform(Bidder agent) {
	        super(agent);
	        testAgent = agent;
	    }
    public void action() {
